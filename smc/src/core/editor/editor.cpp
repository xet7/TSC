...
void cEditor_Item_Object :: Init( cSprite *sprite )
{
	...
	*/
	...
	// create CEGUI link
	CEGUI::String imageset_name = "editor_item " + list_text->getText() + " " + CEGUI::PropertyHelper<unsigned int>::toString( m_parent->getItemCount() );
	m_image = &CEGUI::ImageManager::getSingleton().get( imageset_name );
}
...
void cEditor_Item_Object :: draw( CEGUI::GeometryBuffer &buffer, const CEGUI::Rectf &targetRect, float alpha, const CEGUI::Rectf *clipper ) const
{
	...
	if( m_image && pPreferences->m_editor_show_item_images )
	{
		m_image->render(
			buffer,
			CEGUI::Rectf(targetRect.left() + 15, targetRect.top() + 22, targetRect.left() + 15 + (sprite_obj->m_start_image->m_start_w * preview_scale * global_upscalex), targetRect.top() + 22 + (sprite_obj->m_start_image->m_start_h * preview_scale * global_upscaley) ),
			clipper,
...
void cEditor :: Init( void )
{
	...
	pGuiSystem->getDefaultGUIContext().getRootWindow()->addChild( m_editor_window );

	// Get TabControl
	m_tabcontrol_menu = static_cast<CEGUI::TabControl *>(pGuiSystem->getDefaultGUIContext().getRootWindow()->getChild( "tabcontrol_editor" ));
	// fixme : CEGUI does not detect the mouse enter event if in the listbox or any other window in it
	// TabControl Menu Tab Events
	m_tabcontrol_menu->getTabContents( "editor_tab_menu" )->subscribeEvent( CEGUI::Window::EventMouseEntersSurface, CEGUI::Event::Subscriber( &cEditor::Editor_Mouse_Enter, this ) );
	// TabControl Items Tab Events
	m_tabcontrol_menu->getTabContents( "editor_tab_items" )->subscribeEvent( CEGUI::Window::EventMouseEntersSurface, CEGUI::Event::Subscriber( &cEditor::Editor_Mouse_Enter, this ) );

	// Get Menu Listbox
	m_listbox_menu = static_cast<CEGUI::Listbox *>(pGuiSystem->getDefaultGUIContext().getRootWindow()->getChild( "editor_menu" ));
	// Menu Listbox events
	m_listbox_menu->subscribeEvent( CEGUI::Window::EventMouseEntersSurface, CEGUI::Event::Subscriber( &cEditor::Editor_Mouse_Enter, this ) );
	m_listbox_menu->subscribeEvent( CEGUI::Listbox::EventSelectionChanged, CEGUI::Event::Subscriber( &cEditor::Menu_Select, this ) );
	// Get Items Listbox
	m_listbox_items = static_cast<CEGUI::Listbox *>(pGuiSystem->getDefaultGUIContext().getRootWindow()->getChild( "editor_items" ));
	// Items Listbox events
	m_listbox_items->subscribeEvent( CEGUI::Window::EventMouseEntersSurface, CEGUI::Event::Subscriber( &cEditor::Editor_Mouse_Enter, this ) );
...
void cEditor :: Unload( void )
{
	...
	// close help window
	if( pGuiSystem->getDefaultGUIContext().getRootWindow()->isChild( "editor_help_window" ) )
	{
	...
	if( m_editor_window )
	{
		pGuiSystem->getDefaultGUIContext().getRootWindow()->removeChild( m_editor_window );
...
void cEditor :: Update( void )
{
	...
	if( m_listbox_menu->isVisible() )
	{
		...
		else
		{
			...
			// inactive counter
			else if( Is_Float_Equal( CEGUI::CoordConverter::asRelative( m_editor_window->getXPosition(), 1 ), 0.0f ) )
			{
				// if mouse is over the window
				if( m_tabcontrol_menu->isHit( pGuiSystem->getDefaultGUIContext().getMouseCursor().getPosition() ) )
				{
					m_menu_timer = 0.0f;
				}
				// inactive
...
{
	// already unloaded
	if( !pGuiSystem->getDefaultGUIContext().getRootWindow()->isChild( "editor_items" ) )
	{
		return;
	}
...
{
	CEGUI::Window *window_help = pGuiSystem->getDefaultGUIContext().getRootWindow()->getChild( "editor_help_window" );
	pGuiSystem->getDefaultGUIContext().getRootWindow()->removeChild( window_help );
...
void cEditor :: Parse_Menu_File( fs::path filename )
{
	...
	for(iter=items.begin(); iter != items.end(); iter++) {
		...
		Add_Menu_Object(name, tags, CEGUI::PropertyHelper<CEGUI::Colour>::fromString(color));
	}
}
